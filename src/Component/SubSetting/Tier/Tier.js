...
import { fetchTierData } from '../../Redux/auth/action';
...
const Tier = () => {
    ...
    const TierAllDataShow = async () => {
        try {
            // Dispatch the loginHandler action, which emits the OTP verification event
            await dispatch(fetchTierData(searchAllData, setSearchLoading, selectedFilter, searchLoading, setLoading, setAlldata));
    ...
    const handleInsertData = (values) => {
        ...
        if (defultValue) {
            try {
                socketData.emit('Tier|updateById', { id: idpass, tier });
                socketData.once('Tier|updateById', (response) => {
                    if (response.success) {
            ...
            try {
                socketData.emit('Tier|post', values);
                socketData.once('Tier|post', (response) => {
                    if (response.success) {
                        CustomMessage('success', response?.message);
                        form.resetFields();
                        TierAllDataShow();
    ...
    const validate = (values) => {
        let isValid = true;
        const trimmedtier = values.tier.trim();
        if (!trimmedtier) {
            isValid = false;
            CustomMessage('warning', 'Please Enter a Tier');
        }
        return isValid;
    };
    ...
    const handleDelete = (_id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                socketData.emit('delete|tier', { _id: _id });
                try {
                    socketData.once('delete|tier', (response) => {
                        if (response.success) {
                            CustomMessage('success', response?.message);
                            TierAllDataShow();
                        } else {
    ...
    const handleToggleChange = (_id, newStatus) => {
        ...
        socketData.emit('Tier|updateById', { _id, status: newStatus });
        ...
        CustomMessage('success', "Tier updated successfully");
        ...
        return (
            <>
                {skeletonItems?.map((item) => (
                    <tr key={item}>
                        <td colSpan="7" width="100%" className="skeleton-loader">
                        ...
                        <div class="right-header">
                            <div class="row gx-0">
                                <div class="col-6 d-flex align-items-start">
                                    <h2>{defultValue ? `Update Tier` : `Create Tier`}</h2>
                                </div>
                        ...
                        <div className="body">
                            <div className="d-lg-flex d-md-flex">
                                <div className="form-group sname">
                                    ...
                                    <Form.Item name="tier"
                                        htmlFor="sname" rules={[{ required: true, message: 'Please enter a Tier' }]}>
                                        <Input
                                            type="text" id="sname"
                                            placeholder="Tier"
                                            value={createData?.tier}
                                            className="form-control Short Name"
                                            onChange={(e) => onInputChange(e, 'tier')}
                                        />
                                    </Form.Item>
                                </div>
